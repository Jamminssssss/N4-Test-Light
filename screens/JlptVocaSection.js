...
const VocaSection = ({ navigation }) => {
  ...
  const renderVocabularyItem = ({ item, index }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
    >
      <Text style={[styles.cardText, { color: theme.textColor }]}> {item.japanese} </Text>
      {expandedItems[index] && selectedLanguage !== 'japanese' && (
      ...
      <View style={styles.speakIconContainer}>
        <Ionicons name="volume-high-outline" size={22} color={theme.textColor} />
      </View>
    </TouchableOpacity>
  );
